...
# ONLY import lightweight modules at top level - NO DATABASE OR API IMPORTS
...
async def backup(backup_path):
    ...
    try:
        ...
        click.echo(f"Database backup completed: {backup_path}")
...
def config():
    ...
    click.echo("NFL API Configuration")
    click.echo("-" * 30)
    
    # Lazy import
    from fantasy_ai.core.data.sources.nfl_comprehensive import get_api_config
...
async def full(max_calls, concurrent):
    ...
    try:
        ...
        click.echo("=" * 50)
        click.echo(f"Duration: {metrics.end_time - metrics.start_time}")
        click.echo(f"Players processed: {metrics.total_players_processed}")
        click.echo(f"Stats collected: {metrics.total_stats_collected}")
        click.echo(f"API calls made: {metrics.total_api_calls}")
...
async def validate(player_id, season):
    ...
    try:
        # Lazy import of heavy ML libraries - only loads when this command runs
        ...
        click.echo(f"Anomaly Score: {quality_metrics.anomaly_score:.3f}")
...
async def report():
    ...
    try:
        ...
        async with db_manager.get_session() as session:
            ...
            click.echo(f"Low Quality (<0.6): {low_quality} ({low_quality/total_players:.1%})")
...
async def cleanup():
    ...
    try:
        ...
        if await db_manager.test_connection():
            ...
            click.echo("Database not accessible for optimization")
