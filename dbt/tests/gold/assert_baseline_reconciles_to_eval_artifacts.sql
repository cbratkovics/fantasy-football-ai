...
-- to 1e-4, and its within-3 rate to within one row. One row, not 1e-4, because a trailing mean
...
-- one ulp on such ties (observed: one WR row of 2,391 in the 2024 artifact). Any definitional
-- error moves the rate by far more than a row. Needs the full stats history (the seed of every
-- player's trailing mean), so it is disabled when the warehouse is built from the CI fixture
-- (var full_stats = false).
